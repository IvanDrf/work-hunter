...
func (h *Handler) UpdateProfile(ctx context.Context, req *user_api.UpdateProfileRequest) (*user_api.UserProfile, error) {
	...
	if err != nil {
		...
		log.Error("Unhandled system error during profile updating", slog.String("error", err.Error()))
...
func (h *Handler) UpdateUserStatus(ctx context.Context, req *user_api.UpdateUserStatusRequest) (*user_api.UserProfile, error) {
	...
	if err != nil {
		...
		log.Error("Unhandled system error during profile updating status", slog.String("error", err.Error()))
		return nil, status.Error(codes.Internal, "internal server error")
	}
